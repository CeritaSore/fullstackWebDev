<section class="banner_area">
    <div class="banner_inner d-flex align-items-center">
        <div class="container">
            <div class="banner_content text-center">
                <h2>Portfolio</h2>
                <div class="page_link">
                    <a href="index.php?hal=home">Home</a>
                    <a href="elements.php?hal=portfolio">Portfolio</a>
                </div>
            </div>
        </div>
    </div>
</section>
<!--================Start Portfolio Area =================-->
<section class="portfolio_area mt-5" id="portfolio">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <div class="main_title text-left">
                    <h2>quality work <br>
                        Recently done project </h2>
                </div>
            </div>
        </div>
        <div class="filters portfolio-filter">
            <ul>
                <li class="active" data-filter="*">all</li>

            </ul>
        </div>

        <div class="filters-content">
            <div class="row portfolio-grid justify-content-center">
                <div class="col-lg-4 col-md-6 all latest">
                    <div class="portfolio_box">
                        <div class="single_portfolio">
                            <img class="img-fluid w-100" src="./partition/1.png" alt="">
                            <div class="overlay"></div>
                            <a href="./partition/1.png" class="img-gal">
                                <div class="icon">
                                    <span class="lnr lnr-cross"></span>
                                </div>
                            </a>
                        </div>
                        <div class="short_info">
                            <h4><a href="./partition/portfolioDetail.php">program klasifikasi data kendaraan dengan JavaScript</a></h4>
                            <p>HTML,Bootstrap,JavaScript</p>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 col-md-6 all latest">
                    <div class="portfolio_box">
                        <div class="single_portfolio">
                            <img class="img-fluid w-100" src="./partition/2.png" alt="">
                            <div class="overlay"></div>
                            <a href="./partition/2.png" class="img-gal">
                                <div class="icon">
                                    <span class="lnr lnr-cross"></span>
                                </div>
                            </a>
                        </div>
                        <div class="short_info">
                            <h4><a href="./partition/portfolioDetail.php">program klasifikasi data gempa dengan JavaScript</a></h4>
                            <p>HTML,Bootstrap,JavaScript</p>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 col-md-6 all latest">
                    <div class="portfolio_box">
                        <div class="single_portfolio">
                            <img class="img-fluid w-100" src="./partition/3.png" alt="">
                            <div class="overlay"></div>
                            <a href="./partition/3.png" class="img-gal">
                                <div class="icon">
                                    <span class="lnr lnr-cross"></span>
                                </div>
                            </a>
                        </div>
                        <div class="short_info">
                            <h4><a href="./partition/portfolioDetail.php">program klasifikasi data hewan dan suaranya dengan jquery</a></h4>
                            <p>HTML,Bootstrap,jquery</p>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 col-md-6 all latest">
                    <div class="portfolio_box">
                        <div class="single_portfolio">
                            <img class="img-fluid w-100" src="./partition/4.png" alt="">
                            <div class="overlay"></div>
                            <a href="./partition/4.png" class="img-gal">
                                <div class="icon">
                                    <span class="lnr lnr-cross"></span>
                                </div>
                            </a>
                        </div>
                        <div class="short_info">
                            <h4><a href="./partition/index.php">landing page dengan switch untuk dark mode</a></h4>
                            <p>HTML,Bootstrap,php</p>
                        </div>
                    </div>
                </div>

            </div>
        </div>
    </div>
</section>
<!--================End Portfolio Area =================-->